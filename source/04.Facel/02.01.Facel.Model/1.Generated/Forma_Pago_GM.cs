//------------------------------------------------------------------------------
// <auto-generated>
//     This code was generated from a template.
//
//     Changes to this file may cause incorrect behavior and will be lost if
//     the code is regenerated.
// </auto-generated>
//------------------------------------------------------------------------------

using System;
using System.Collections.Generic;
using System.Data;
using System.Data.Common;
using System.Collections.ObjectModel;
using Library.Common.Data;
using Library.Framework;
using Library.Framework.Layers;
using Library.Framework.Data;

using Facel.Data.Entities;
using Facel.Data.Accesses;
using Facel.Data.Logics;
using Facel.Business.Entities;
using Facel.Business.Accesses;
using Facel.Business.Logics;
using Facel.Join.Entities;
using Facel.Join.Accesses;
using Facel.Join.Logics;
using Facel.Filter.Entities;
using Facel.Filter.Accesses;
using Facel.Filter.Logics;
using Facel.Main;


//DataEntity
namespace Facel.Data.Entities
{
    [Serializable]
    public partial class Forma_Pago_DE : EntityId
    {
        protected internal string _Nombre;
        public virtual string Nombre
    	{
    	    get { return _Nombre; }
    	    set { if (value != _Nombre) { _Nombre = value; OnPropertyChanged("Nombre"); } }
        }	
        protected internal Nullable<int> _Dias;
        public virtual Nullable<int> Dias
    	{
    	    get { return _Dias; }
    	    set { if (value != _Dias) { _Dias = value; OnPropertyChanged("Dias"); } }
        }	
        protected internal Nullable<bool> _EsDefault;
        public virtual Nullable<bool> EsDefault
    	{
    	    get { return _EsDefault; }
    	    set { if (value != _EsDefault) { _EsDefault = value; OnPropertyChanged("EsDefault"); } }
        }	
        protected internal Nullable<bool> _Activo;
        public virtual Nullable<bool> Activo
    	{
    	    get { return _Activo; }
    	    set { if (value != _Activo) { _Activo = value; OnPropertyChanged("Activo"); } }
        }	
    
    
    
    }
}

namespace Facel.Business.Entities
{
    [Serializable]
    public partial class Forma_Pago_BE : Forma_Pago_DE
    {
    }
}

//DataAccess
namespace Facel.Data.Accesses
{
    [Serializable]
    public partial class Forma_Pago_DA : AccessEntityId
    {
    	public  Forma_Pago_DA(string paramTableName, string paramConnectionStringName)
            : base(paramTableName, paramConnectionStringName)
    	{
    	}
    
        protected override Entity GetDE()
        {
            return new Forma_Pago_DE();
        }
    
        public override List<Field> GetFieldsData(Entity paramDE, bool paramIsSourceColumn = false, 
    		string paramTableAlias = "")
        {
            Forma_Pago_DE be = (Forma_Pago_DE)paramDE;
    
            List<Field> fields = new List<Field>();
    
            fields = AddField<string>(fields, "nombre", System.Data.DbType.String, be.Nombre, paramIsSourceColumn, paramTableAlias);
            fields = AddField<Nullable<int>>(fields, "dias", System.Data.DbType.Int32, be.Dias, paramIsSourceColumn, paramTableAlias);
            fields = AddField<Nullable<bool>>(fields, "es_default", System.Data.DbType.Boolean, be.EsDefault, paramIsSourceColumn, paramTableAlias);
            fields = AddField<Nullable<bool>>(fields, "activo", System.Data.DbType.Boolean, be.Activo, paramIsSourceColumn, paramTableAlias);
            
    		return fields;
        }
    
        protected override Entity LoadDataReader(DbDataReader paramReader, Entity paramDE)
    	{
            Forma_Pago_DE be = (Forma_Pago_DE)paramDE;
    
    		be._Nombre = Conversion.To<string>(paramReader[TableColumn("nombre")]);
    		be._Dias = Conversion.To<Nullable<int>>(paramReader[TableColumn("dias")]);
    		be._EsDefault = Conversion.To<Nullable<bool>>(paramReader[TableColumn("es_default")]);
    		be._Activo = Conversion.To<Nullable<bool>>(paramReader[TableColumn("activo")]);
      
            return be;
    	}
        protected override Entity LoadDataRow(DataRow paramRow, Entity paramDE)
    	{
            Forma_Pago_DE be = (Forma_Pago_DE)paramDE;
    
    		be._Nombre = Conversion.To<string>(paramRow[TableColumn("nombre")]);
    		be._Dias = Conversion.To<Nullable<int>>(paramRow[TableColumn("dias")]);
    		be._EsDefault = Conversion.To<Nullable<bool>>(paramRow[TableColumn("es_default")]);
    		be._Activo = Conversion.To<Nullable<bool>>(paramRow[TableColumn("activo")]);
      
            return be;
    	}
        protected override Entity LoadDataParameters(DbParameterCollection paramParameters, Entity paramDE)
    	{
            Forma_Pago_DE be = (Forma_Pago_DE)paramDE;
    
    		be._Nombre = Conversion.To<string>(paramParameters[TableParameter("nombre")].Value);
    		be._Dias = Conversion.To<Nullable<int>>(paramParameters[TableParameter("dias")].Value);
    		be._EsDefault = Conversion.To<Nullable<bool>>(paramParameters[TableParameter("es_default")].Value);
    		be._Activo = Conversion.To<Nullable<bool>>(paramParameters[TableParameter("activo")].Value);
      
            return be;
    	}
    
        protected override Entity ClearData(Entity paramDE)
        {
            Forma_Pago_DE be = (Forma_Pago_DE)paramDE;
    
    		be._Nombre = null;
    		be._Dias = null;
    		be._EsDefault = null;
    		be._Activo = null;
      
    
            return be;
        }
    
    }
}

namespace Facel.Business.Accesses
{
    [Serializable]
    public partial class Forma_Pago_BA : Forma_Pago_DA
    {
    	public  Forma_Pago_BA(string paramTableName, string paramConnectionStringName)
            : base(paramTableName, paramConnectionStringName)
    	{
    	}
    	
    	protected override Entity GetDE()
        {
            return new Forma_Pago_BE();
        }
    }
}

//DataLogic
namespace Facel.Data.Logics
{
    [Serializable]
    public partial class Forma_Pago_DL : LogicEntity
    {
        protected override Access GetDA()
        {
            return new Forma_Pago_DA(TableName, ConnectionStringName);
        }
    	
    	protected override bool ValidateInsertModel(Entity paramDE, ref string paramField, ref string paramMessage)
        {
            Forma_Pago_BE be = (Forma_Pago_BE)paramDE;
    
      
    		return base.ValidateInsertModel(paramDE, ref paramField, ref paramMessage);
        }
    	protected override bool ValidateDeleteModel(Entity paramDE, ref string paramField, ref string paramMessage, bool paramValidateCascadeIntegrity = true)
        {
            Forma_Pago_BE be = (Forma_Pago_BE)paramDE;
    
            if (paramValidateCascadeIntegrity)
            {
            }
    			     
            return base.ValidateDeleteModel(paramDE, ref paramField, ref paramMessage, paramValidateCascadeIntegrity);
        }
    	
    	public override byte EraseModel(Entity paramDE, bool paramCheckKeyEmpty = true, bool paramIsSourceColumn = false)
        {
            byte ret = 1;
    
    		Forma_Pago_BE be = (Forma_Pago_BE)paramDE;
    
    		foreach (Forma_Pago_BE e in new Forma_Pago_FL().LoadConvert(new Forma_Pago_FL().Convert(be), 1))
    		{
     		}
    		
    		if (ret == 1)
                ret = base.EraseModel(paramDE, paramCheckKeyEmpty, paramIsSourceColumn);
            
    		return ret;
        }	
    }
}

namespace Facel.Business.Logics
{
    [Serializable]
    public partial class Forma_Pago_BL : Forma_Pago_DL
    {
        protected override Access GetDA()
        {
            return new Forma_Pago_BA(TableName, ConnectionStringName);
        }	
    }
    
}

//JoinEntities
namespace Facel.Join.Entities
{
    [Serializable]
    public partial class Forma_Pago_JE : EnumerateId
    {
        public virtual Filter<string> Nombre { get; set; }
        public virtual Filter<Nullable<int>> Dias { get; set; }
        public virtual Filter<Nullable<bool>> EsDefault { get; set; }
        public virtual Filter<Nullable<bool>> Activo { get; set; }
    
    }
}

namespace Facel.Filter.Entities
{
    [Serializable]
    public partial class Forma_Pago_FE : Forma_Pago_JE
    {
    }
}


//JoinAccess
namespace Facel.Join.Accesses
{
    [Serializable]
    public partial class Forma_Pago_JA : AccessEnumerateId
    {	
    	public  Forma_Pago_JA(string paramTableName, string paramConnectionStringName)
            : base(paramTableName, paramConnectionStringName)
    	{
    	}
    	
    	protected override Entity GetDE()
        {
            return new Forma_Pago_DE();
        }
    	protected override Access GetDA()
        {
            return new Forma_Pago_DA(TableName, ConnectionStringName);
        }
    	
    	public override List<Field> GetFieldsData(Enumerate paramDE, bool paramIsSourceColumn = false, 
    		string paramTableAlias = "")
        {
            Forma_Pago_JE be = (Forma_Pago_JE)paramDE;
    
            List<Field> fields = new List<Field>();
    
            fields = AddField<string>(fields, "nombre", System.Data.DbType.String, be.Nombre, paramIsSourceColumn, paramTableAlias);
            fields = AddField<Nullable<int>>(fields, "dias", System.Data.DbType.Int32, be.Dias, paramIsSourceColumn, paramTableAlias);
            fields = AddField<Nullable<bool>>(fields, "es_default", System.Data.DbType.Boolean, be.EsDefault, paramIsSourceColumn, paramTableAlias);
            fields = AddField<Nullable<bool>>(fields, "activo", System.Data.DbType.Boolean, be.Activo, paramIsSourceColumn, paramTableAlias);
            
    		return fields;
        }
    	
    	protected override List<Field> GetFieldsJoin(List<Field> paramFields, Enumerate paramDE, int paramDepth, int paramMaxDepth = 0, bool paramIsSourceColumn = false, 
    		string paramTableAlias = "")
        {
            Forma_Pago_FE be = (Forma_Pago_FE)paramDE;
    		
            
    		return paramFields;
    	}
    	protected override string GetTablesJoin(string paramTables, Enumerate paramDE, int paramDepth, int paramMaxDepth = 0, bool paramIsSourceColumn = false, 
    		string paramTableAlias = "",
    		Dictionary<string, string> paramFieldsJoin = null)
        {
    		Forma_Pago_FE be = (Forma_Pago_FE)paramDE;	
    
    
    		return paramTables;
    	}	
    		
    }
}

namespace Facel.Filter.Accesses
{
    [Serializable]
    public partial class Forma_Pago_FA : Forma_Pago_JA
    {	
    	public  Forma_Pago_FA(string paramTableName, string paramConnectionStringName)
            : base(paramTableName, paramConnectionStringName)
    	{
    	}
    	
    	protected override Entity GetDE()
        {
            return new Forma_Pago_BE();
        }
    	protected override Access GetDA()
        {
            return new Forma_Pago_BA(TableName, ConnectionStringName);
        }
    }
}


//JoinLogic
namespace Facel.Join.Logics
{
    [Serializable]
    public partial class Forma_Pago_JL : LogicEnumerate
    {
    	protected override Access GetDA()
        {
            return new Forma_Pago_JA(TableName, ConnectionStringName);
        }
    	
    	public ObservableCollection<Forma_Pago_BE> LoadConvert(Enumerate paramDE, int paramMaxDepth = 0, TypeLoad paramTypeLoad = TypeLoad.DataReader, bool paramIsSourceColumn = false,
                int paramTop = 0,
    			int paramRowFrom = 0, int paramRowTo = 0)
        {
        	return new ObservableCollection<Forma_Pago_BE>(Load(paramDE, paramMaxDepth, paramTypeLoad, paramIsSourceColumn,
                    paramTop,
    				paramRowFrom, paramRowTo).ConvertAll(x => x as Forma_Pago_BE));
        }
    	public Forma_Pago_FE Convert(Forma_Pago_BE paramDE)
        {
            Forma_Pago_FE be = new Forma_Pago_FE();
    
    		be.Id = new Filter<Nullable<int>>(paramDE.Id);
    		be.Nombre = new Filter<string>(paramDE.Nombre);
    		be.Dias = new Filter<Nullable<int>>(paramDE.Dias);
    		be.EsDefault = new Filter<Nullable<bool>>(paramDE.EsDefault);
    		be.Activo = new Filter<Nullable<bool>>(paramDE.Activo);
      
            return be;
        }	
    }
}

namespace Facel.Filter.Logics
{
    [Serializable]
    public partial class Forma_Pago_FL : Forma_Pago_JL
    {
    	protected override Access GetDA()
        {
            return new Forma_Pago_FA(TableName, ConnectionStringName);
        }
    }
}

